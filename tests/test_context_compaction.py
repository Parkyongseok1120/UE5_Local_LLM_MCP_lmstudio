...
def test_budget_decision_reserves_build_output() -> None:
...
def test_checkpoint_keeps_facts_but_drops_tool_directives() -> None:
    ...
    assert "requiredNextTool" not in checkpoint
    assert "requiredNextToolArgs" not in str(checkpoint)
...
def test_hard_compaction_removes_a_single_huge_old_message() -> None:
    ...
    assert "authoritative" not in str(compacted)
