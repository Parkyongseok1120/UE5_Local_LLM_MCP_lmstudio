...
another action useful. After a timeout such as `-32001`, verify observable state
before deciding whether changed arguments, another tool, a fresh session, or a
clear limitation is the best next step.

Keep build/log/write/validation evidence summary-first. Build and Automation use
one bounded process owner; timeout terminates the process tree, and
`fullLogPath` may contain bounded head/tail output rather than unlimited raw
output. Use `target=Editor` as the selected project's portable Editor-target
alias and preserve explicit non-Editor targets. Never claim compile success
without build evidence or asset/runtime success without the corresponding
Editor evidence.
