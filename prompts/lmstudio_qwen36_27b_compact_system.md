...
- **Write auth is automatic for one active task:** normally omit `taskAuthorization`; the server binds the current project route and refreshes it after gates. If the server reports ambiguous tasks or explicitly requests complete authorization, reuse the returned object unchanged and never invent fields. On `TASK_ROUTE_STALE`, retry once as instructed; do not replan. If `pendingGates` is non-empty, call the first named gate; do not call `unreal_agent_plan` to bypass it.
