...
- **Write auth (7 fields, server-issued only):** never invent placeholder IDs/tokens. After any gate, copy `gateCompletion.taskAuthorization` unchanged — never reuse the original plan auth. If no server-issued auth exists, call `unreal_agent_plan` once. On `TASK_ROUTE_STALE`, retry once with returned `taskAuthorization`; do not replan.
