...
- For brand-new files, pass concrete `targetFiles` to `unreal_code_sketch_claim_validate` before writing. Use `changeKind=new_file` for exactly one new target and `changeKind=multifile` for a new `.h`/`.cpp` pair.
