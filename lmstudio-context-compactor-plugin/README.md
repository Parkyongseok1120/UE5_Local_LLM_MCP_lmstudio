...
라운드별 컨텍스트 측정값은 기본적으로 채팅에 표시하지 않습니다. 문제를 진단할 때만 `Show debug info`를 켭니다.
...
설치기는 잠금 파일대로 패키지를 준비하고 검사·빌드 후 `lms dev --install -y`로 등록합니다. 이름·소유자·revision과 `.lmstudio/production.js`가 있는지 확인합니다. 현재 버전은 0.4.58 / revision 105입니다.
