...
def attach_control_envelope(
    ...
    *,
    ...
    task_auth = result.get("taskAuthorization") if isinstance(result.get("taskAuthorization"), dict) else {}
    non_authoritative_task_id = str(
    ...
    if non_authoritative_task_id:
        # Python's phase_tool_router reducer is the sole owner of task control
        # v2. A response adapter may diagnose a missing committed envelope, but
        # it must not recover semantics from legacy action/status fields.
        result["control"] = {
            "version": 1,
            "taskId": non_authoritative_task_id,
            "phase": str(tool_name or result.get("phase") or "task_control_missing"),
