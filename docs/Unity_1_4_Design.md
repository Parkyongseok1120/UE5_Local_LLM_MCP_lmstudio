...
- Protocol `1`, bridge/server `1.4.0-beta.3`. Local TCP, IPv4 loopback, OS-selected port, one bounded newline-delimited JSON request/reply per connection. No HTTP/browser origin surface. Discovery is under `Library/EvidenceFirst/bridge.json` with a random token, process ID, canonical root and session identity. Tokens are transport-only, never returned in MCP results.
