...
**Status: NOT RELEASE-READY.** Product label is aligned to **1.3.0 RC3**, but this candidate is **not** approved for public distribution until Windows physical install validation and the remaining blockers below are closed. `installer/manifest.json` → `portablePackage.releaseReady` remains **`false`**.
...
Do not advertise RC3 as deployable until `installer/manifest.json` → `portablePackage.releaseReady` is set `true` after Windows physical verification and the remaining release gates pass.
