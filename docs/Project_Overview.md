...
See [1.3.0 RC3 draft notes](Release_Notes_1_3_0_RC3.md) for the evidence boundary and component versions.
...
현재 제품 라벨은 **1.3.0 RC3**입니다(프리릴리스; 설치 검증 전까지 배포 불가). 범용 evidence-first 분석, 강화된 architecture/code-generation guardrail, cross-platform 통합 설치, 안정성 중심의 task/context 복구, hardened sketch claim 검증, write-path mutation semantic guard를 포함하도록 1.3 prerelease 라인을 발전시킵니다.
...
근거 범위와 컴포넌트 버전은 [1.3.0 RC3 초안 노트](Release_Notes_1_3_0_RC3.md)를 참고하세요.
