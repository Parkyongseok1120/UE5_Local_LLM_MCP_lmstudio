...
v1.2.5 closed the compile-fix stabilization line with stronger write safety, scoped validate-on-write rollback, loop breakers, and expanded static rules. The final v1.2.5 hotfix also shipped **partial** LM Studio context/UX mitigations; the rows below remain because full automation still needs a v1.3.0 design pass.

**Partially delivered in v1.2.5 final hotfix (LM Studio chat path):**

- Compact `build_unreal_project` / `read_unreal_logs` responses with `fullLogPath` artifacts
- `write_session_handoff` + `.agent/handoff/latest.md` resume checklist
- `get_workspace_info.contextHygiene` guidance and `MCP_AGENT_RESULT_MAX_CHARS` safety ceiling
- Sketch validator `verdictSummary` + `replacement`, denylist/static warnings for common invented UE APIs

| Gap | v1.2.5 state | Planned work | Target |
|---|---|---|---|
| Long-session context collapse (~60K tokens) | **Partial:** manual handoff tool + hygiene guidance; LM Studio still keeps full chat history with no automatic compaction. | Automatic session budgeting, risk-triggered checkpoint enforcement, token-use diagnostics. | v1.3.0 |
| UBT / build log token bloat | **Partial:** compact MCP responses + log file artifacts; wrapper retry feedback still verbose in some paths. | Unified compact error slices across wrapper + MCP + retry loops. | v1.3.0 |
...
| UE version-pinned API hallucination (real-project compile failures) | **Partial:** denylist + sketch validator + advisory static warnings + RAG blocklist for DisableGravity/GetURL/SpawnActor&/GEngine; include-before-use and failure-memory still open. | Include-before-use rule for project `U*` types, failure-memory capture from real UBT logs, broader engine-symbol allowlist. | v1.3.0 |
...
v1.2.5는 write safety, validate-on-write 스코핑, loop breaker, static rule 확장으로 compile-fix 안정화 라인을 마쳤습니다. 최종 v1.2.5 hotfix에서 LM Studio context/UX 완화를 **부분 배포**했고, 아래 항목은 자동화/transport 수준 변경이 남아 v1.3.0으로 이월됩니다.

**v1.2.5 final hotfix에서 부분 배포 (LM Studio chat):**

- compact `build_unreal_project` / `read_unreal_logs` + `fullLogPath` artifact
- `write_session_handoff` + `.agent/handoff/latest.md` resume checklist
- `get_workspace_info.contextHygiene` + `MCP_AGENT_RESULT_MAX_CHARS` 상한
- sketch validator `verdictSummary` + `replacement`, 흔한 invented UE API denylist/static warning

| Gap | v1.2.5 상태 | 계획 작업 | 목표 버전 |
|---|---|---|---|
| 긴 세션 컨텍스트 붕괴 (~60K tokens) | **부분:** handoff tool + hygiene guidance; LM Studio는 여전히 전체 채팅 히스토리를 자동 압축하지 않음. | 자동 session budgeting, risk checkpoint 강제, token-use 진단. | v1.3.0 |
| UBT / build log 토큰 비대 | **부분:** compact MCP 응답 + log artifact; wrapper retry feedback 일부 경로는 여전히 verbose. | wrapper + MCP + retry loop 전반의 compact error slice 통일. | v1.3.0 |
...
| UE 버전 고정 API 환각 (실프로젝트 compile 실패) | **부분:** denylist + sketch validator + advisory static warning + RAG blocklist(DisableGravity/GetURL/SpawnActor&/GEngine); include-before-use와 failure-memory는 미완. | 프로젝트 `U*` 타입 include-before-use, 실 UBT log failure-memory, engine-symbol allowlist 확장. | v1.3.0 |
