...
The current product is **1.3.0 RC2**, but it has no new paired live-model score yet. Do not attribute the v1.2.5 measurements below to Beta4/Beta5/RC1/RC2 architecture or orchestration changes. RC2 makes local-model context retention and exact tool-call generation a more visible bottleneck after deterministic server handoffs, but that observation is not a quantified model regression or improvement.
