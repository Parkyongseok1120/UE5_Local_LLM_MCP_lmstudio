...
| **Evidence-First MCP server** | 1.1.0 | [`evidence_first_mcp.py`](../skills/evidence-first-code-audit/scripts/evidence_first_mcp.py) | Read-only audit contract, validation schema, and MCP protocol metadata |
...
| **Portable manifest** | 2.1.13 | [`installer/manifest.json`](../installer/manifest.json) | Portable ZIP bundle metadata (layout + required files) |
...
Current `main` keeps the product label at 1.3.1 and advances the Evidence-First
MCP server to **1.1.0**, the context compactor to **0.4.51/revision 98**, and the
portable manifest to **2.1.13**. This post-release payload keeps the Python-free
launcher seed path, makes the validator's exact packet contract public, removes
the redundant nested compaction enable gate, rechecks pressure during multi-round
tool work, and preserves bounded validation-repair feedback. The installer still
never enables LM Studio's host-owned chat toggle. The published `v1.3.1` tag and
its 2.1.11 artifact remain immutable.
