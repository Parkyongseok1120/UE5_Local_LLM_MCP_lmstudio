...
The current product is **1.3.0 RC1**, but no paired RC1 live-model rerun has been completed. All scores on this page remain historical v1.2.5 baselines until a new artifact is published.
...
- Qwen 3.5 9B (`20260711-090534-qwen35-9b`, deepseek-v4-flash profile) reached **36/36 Pass@K** and **35/36 Pass@1** after scoped write stabilization; remaining Pass@1 gap is one include-registration case with validation-reject retries.
- Qwen 3.5 9B's prior gap (`local_lnk2019_missing_cpp_definition`) cleared in the 20260711 run.
...
현재 제품은 **1.3.0 RC1**이지만 RC1 paired live-model 재측정은 아직 완료되지 않았습니다. 새로운 artifact가 공개되기 전까지 이 문서의 모든 점수는 historical v1.2.5 baseline입니다.
...
- Qwen 3.5 9B (`20260711-090534-qwen35-9b`, deepseek-v4-flash profile)는 scoped write stabilization 이후 **36/36 Pass@K**, **35/36 Pass@1**을 기록했습니다. 남은 Pass@1 gap은 include-registration 1건의 validation-reject retry입니다.
- 이전 9B 약점이었던 `local_lnk2019_missing_cpp_definition`은 20260711 run에서 회복됐습니다.
