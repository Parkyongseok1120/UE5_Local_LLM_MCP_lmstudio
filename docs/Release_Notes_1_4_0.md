...
1.4.0은 Unity용 MCP와 Editor Bridge beta 1을 통합 배포본에 추가하고, LM Studio 대화 압축기의 프로젝트 구분·응답 표시·연속성 보존을 개선합니다. Unreal Direct 도구와 Unity 도구는 확인된 프로젝트 종류에 따라 분리해 노출합니다.
...
| 통합 제품 | 1.4.0 (`v1.4.0`) |
| 휴대용 배포 구성 | 2.1.18 |
| Unreal Node MCP | 0.3.22 |
| Unity MCP·Editor Bridge | 1.4.0-beta.1 |
| 근거 검토 서버 | 1.1.1 |
| 대화 압축기 | 0.4.58 / revision 105 |

## Unity beta 1 범위

Unity 구성 요소는 통합 제품 1.4.0에 포함되는 beta 1입니다. 구현 기준은 Unity 2022.3 이상에서 제공되는 공통 Editor API이며, 실제 Editor 통합 검증은 Unity 6000.3.14f1 Intel macOS 중심입니다. 다른 Unity 버전과 Windows·Linux Editor의 전체 통합 실행을 보장하지 않습니다.
