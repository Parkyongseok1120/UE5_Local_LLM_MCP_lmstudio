...
def test_checkpoint_rebase_does_not_call_a_removed_node_journal_controller(
    tmp_path: Path,
    monkeypatch,
) -> None:
    monkeypatch.setenv("AGENT_STATE_ROOT", str(tmp_path / "state"))
    ...
    assert "recoveryJournalResolution" not in rebased
    state = task_status(tmp_path, started["taskSessionId"])["state"]
    assert "recoveryObligation" not in state
    assert "lastRecoveryJournalResolution" not in state
