#!/usr/bin/env python
"""Engine-portability regressions for legacy diagnostics exposed to Direct clients.

The default Direct servers do not expose the historical code-sketch gate.  These
tests keep the useful exact-engine resolution coverage while ensuring that the
compatibility diagnostic cannot leak its former route/write-gate authority into
a Direct response.
"""
...
def test_claim_validation_error_is_diagnostic_not_a_direct_write_gate(
    monkeypatch,
    tmp_path: Path,
) -> None:
    ...
    assert "gatePassed" not in payload
    assert "writeGateClosed" not in payload
