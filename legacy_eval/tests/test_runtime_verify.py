...
def test_runtime_verify_plan_is_diagnostic_without_server_owned_next_action(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    assert "nextActionArgs" not in payload
    assert "nextActionIsTool" not in payload
    environment = payload["runtimeVerifyPlan"]["environment"]
    assert environment["projectFile"] == str(project.resolve())
    assert environment["engineRoot"] == str(engine.resolve())
    assert environment["editorCmd"] == str(editor.resolve())
    assert environment["exactEngineBinding"] is True
