...
The current product is **1.3.0 RC3**, but it has no new paired live-model score yet. Do not attribute the v1.2.5 measurements below to Beta4/Beta5/RC1/RC2/RC3 architecture or orchestration changes. RC3 makes local-model context retention and exact tool-call generation a more visible bottleneck after deterministic server handoffs, but that observation is not a quantified model regression or improvement.
...
Sonnet 5 is only a historical comparison label in older evaluation material; it is not a current workflow contract or a model-equivalence claim.
...
Compact-model optimization tracks include Qwen 3.5 9B, Qwen3.5-9B-DeepSeek-V4-Flash-GGUF, GPT OSS 20B, and gpt-oss-20b-claude-opus-sonnet-reasoning-i1-GGUF community fine-tunes. See [Model measurement results](Model_Measurement_Results.md) and [Model profiles](Model_Profiles.md).
...
| `qwen3_5_9b` / `qwen3.5-9b-deepseek-v4-flash` | **Measured** — 36-case UE 5.8 live holdout `20260711-090534-qwen35-9b` (post scoped-write-stabilization) | Pass@K **36/36** (100%), Pass@1 **35/36** (97.2%). Multifile **12/12 Pass@1**, module_fix **10/10 Pass@1**, editor_runtime_boundary **1/1 Pass@1**. Pass@1 miss: `local_component_registration_missing_include`. Prior failure `local_lnk2019_missing_cpp_definition` recovered. | **Upper Sonnet 3.7 / lower Sonnet 4 workflow band** for this narrow loop on compact VRAM. |
| `qwen3_5_9b` / `qwen3.5` | **Measured** — prior 36-case UE 5.8 live holdout `20260709-153021-qwen35-9b` (v1.2.5) | Pass@K **35/36** (97.2%), Pass@1 **33/36** (91.7%). Failed `local_lnk2019_missing_cpp_definition`. | Prior compact baseline before scoped-write-stabilization live revalidation. |
