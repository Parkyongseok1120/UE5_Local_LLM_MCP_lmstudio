...
> **ARCHIVED BASELINE.** This scorecard predates the small Direct RAG/Node Direct architecture. Named Python controller and background-job files below are historical evidence, not packaged runtime components or supported MCP entrypoints. Use the root README and `LMStudio_MCP_Tool_Discipline.md` for the current contract.
