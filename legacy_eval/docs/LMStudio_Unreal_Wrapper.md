...
The wrapper remains an explicit CLI workflow. Its former model-driving background-job tools are not exposed by the eight-tool Direct RAG server or by the supported Node Strict server, and the old monolithic Python MCP entry is omitted from portable packages.

Use `.\rag.ps1 wrapper` when a blocking automated wrapper run is deliberately required. For ordinary MCP chat, use factual Direct RAG evidence and the immediate Node `build_unreal_project` capability; the selected model remains responsible for any follow-up reads or edits.
