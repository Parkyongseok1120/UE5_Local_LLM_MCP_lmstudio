# Historical Agent Orchestrator

> **ARCHIVED, UNSUPPORTED PYTHON CONTROLLER.** This planner is retained only for repository-local wrapper/evaluation research. No supported MCP entry exposes it, and the portable package omits the monolithic Python controller. `unreal_agent_plan`, write gates, required tool order, checkpoints, and stop/retry policy below do not authorize or sequence Direct tools. The sole supported Strict implementation is Node `strict_begin`, which does not consume this protocol.
...
No supported MCP entry exposes `unreal_agent_plan`. The historical repository-local implementation returned read-only JSON with:
