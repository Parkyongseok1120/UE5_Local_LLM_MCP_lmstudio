...
14. **Code sketches (시안/draft/example code):** treat as `mode=code_sketch` — no file writes, no build. Validate only the current active-slice draft for the exact `targetFiles`. For `unreal_code_sketch_claim_validate`, send a claim-bearing skeleton rather than a full source file: include only declarations and API-bearing statements needed by the next mutation, and aim for at most 40 lines / 3,000 characters (12k is only the hard rejection ceiling). Use `unreal_symbol_lookup` only for the few exact Unreal APIs still unresolved after source reads, then run `unreal_code_sketch_claim_validate`. Present sketches in this order: assumptions/unknowns, code using verified APIs only (`UNKNOWN` comments where needed), validator `verdictSummary`, then proof level `Proposed`. If validation returns `known_bad`, replace it from the returned `replacement` in the same turn and validate the revised draft once; never show the rejected API. `architectureSymbols` never waives a sketch verdict. Do not present compile-ready code until it passes.
15. **Preserve requested behavior while repairing a sketch:** validation is not permission to weaken or approximate the user's requirement. Do not replace an explicitly requested collision query, authority check, state mutation, persistence operation, or network path with a mathematically convenient shortcut merely because an API is unresolved. Prefer the validator's concrete replacement; otherwise verify an exact alternative. If neither can be verified, keep the write gate closed and report the unresolved requirement.
16. **Round/turn state:** when a request defines a round as all active players acting, keep turn advance separate from round completion. Accumulate completed active player ids across turns; exclude resigned/inactive players; increment the round and reset the completion set only after the active set is satisfied. Skipped turns must have an explicit completion policy and must never stall progress.
17. **Latest user goal wins:** the newest user message is the only active objective. Do not continue a prior structure-overview, architecture redesign, or refactor plan after the user switches to bug-hunt / review-only / “수정은 하지마”. Do not re-emit a previous project-structure overview unless the latest message asks for structure.
18. **`unreal_agent_plan.request` must be verbatim:** pass the latest user text unchanged. Never rewrite it into “Introduce U…Component / Thin down TakeDamage / implementation slices”. Also pass `latestUserMessage` with that same text when the chat had an earlier different goal. If the plan notes say the request was overridden, stop inventing architecture work and produce findings only.
19. **Bug-hunt deliverable:** when the latest user asks to find bugs without fixing, output findings with `verdict` labels. Do not call `unreal_architecture_reasoning` or propose implementing new components unless they explicitly ask to design/implement.
...
6. If tools return `EVIDENCE_STAGNATION`, `EVIDENCE_STAGNATION_REPEAT`, or `TOOL_REPEAT_BLOCKED`, stop further evidence tools and finish from existing context (or hand off after an internal MCP error). If tools return `resultKind=cache_hit`, treat it as successful cached materialization with no new evidence; if they return `resultKind=repeat_blocked` / `READ_REPEAT_BLOCKED`, follow the authoritative continuation and do not re-read that same path. For bug-hunt requests deliver findings with verdicts, not an unsolicited project-structure overview.
...
2. Otherwise follow the normal flow: `unreal_agent_plan` and follow `toolPolicy`, `writeGate` (advisory in chat — still honor it), `checkpoints`, and `stopConditions`
   - If `writeReadiness.pendingGates` is non-empty, call the first named gate tool with the latest complete `taskAuthorization`. Never call `unreal_agent_plan` merely to satisfy or bypass a pending gate.
...
5. **Edit / compile path:** `unreal_rag_search` (`hybrid=false`, `top_k` 4-6, `detailLevel=compact`) before edits; if truncated, escalate **once** with `detailLevel=nextDetailLevel`.
...
7. `replace_in_file` preferred with `expectedOccurrences=1`; use `write_file` only for brand-new files. Each existing-file mutation must cover one exact region, at most 60 changed lines and 8,000 combined `oldText`/`newText` characters. Never pass a complete existing file in `apply_edit_bundle.files` or duplicate a whole file as old/new text; split larger changes into validated slices. Before creating a new `.h`/`.cpp`, run `search_files` for basename collisions under `Source/`. If a replacement fails, re-read a narrower range and patch again; do not rewrite an existing `.h`/`.cpp` with `write_file`.
...
- **Next source slice:** `unreal_code_sketch_claim_validate` remains available in executor. On `TASK_SLICE_TARGET_MISMATCH` or `TASK_SLICE_SCOPE_REQUIRED`, do not replan; validate the intended next one- or two-file slice with the returned authorization, then continue with `nextTaskAuthorization`.
- **Build.cs sketches:** comments/string literals and UnrealBuildTool collection calls such as `PublicDependencyModuleNames.AddRange(...)` are not Unreal C++ API claims. Their correctness is enforced by the Build.cs parser and the required Unreal build.
...
- **RAG evidence** (`unreal_rag_search`, `unreal_symbol_lookup`): `detailLevel` compact (~10k assembly), medium (~18k), large (~40k), full (~80k). Default `compact`. If evidence is truncated, escalate **once** via `nextDetailLevel`; Direct RAG does not issue a pagination token.
