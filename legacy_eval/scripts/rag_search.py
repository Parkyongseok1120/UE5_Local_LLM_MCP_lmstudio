#!/usr/bin/env python
"""Archived monolithic retrieval/controller stack."""
...
def search(index: Path, query: str, top_k: int, options: SearchOptions | None = None) -> list[dict[str, Any]]:
    ...
    if (
        mode in {"compile_fix", "module_fix", "reflection_fix", "multifile_refactor"}
        and not options.evidence_only
    ):
        try:
