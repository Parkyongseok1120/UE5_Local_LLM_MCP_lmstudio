...
def system_prompt_static_prefix(rules_text: str, edit_limits: dict[str, Any] | None = None) -> str:
    ...
    prompt_key = (
        ...
        if os.environ.get("MCP_EXECUTION_MODE", "direct").strip().casefold() == "strict"
        else "recommendedSystemPrompt"
    )
    recommended = str(
        limits.get(prompt_key) or limits.get("recommendedSystemPrompt") or ""
    ).strip()
    if recommended:
...
def run(args: argparse.Namespace) -> int:
    ...
    while True:
        ...
        if configured_context_length > 0:
            strict_wrapper_mode = (
                os.environ.get("MCP_EXECUTION_MODE", "direct").strip().casefold()
                == "strict"
            )
            ...
            context_decision = context_budget_decision(
                ...
                next_tool_name=(
                    "build_unreal_project" if last_build_records else "read_file_range"
                ) if strict_wrapper_mode else "",
            )
            if context_decision.action != "normal":
                ...
                checkpoint["buildState"] = (
                    ...
                    if strict_wrapper_mode
                    else {
                        "attempt": attempt,
                        "latestBuildDiagnostics": last_build_records[-16:],
                    }
                )
