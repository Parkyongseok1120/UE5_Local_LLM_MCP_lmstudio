#!/usr/bin/env python
"""Historical Python workflow server; unsupported by the Direct installation.

The supported MCP entry is ``unreal_rag_direct.py``.  This module remains only
for repository-local regression archaeology and requires an explicit legacy
CLI acknowledgement before it can open an MCP transport.
"""
...
def _server_direct_mode(server: Any) -> bool:
    """Read the process mode without requiring a fully initialized test server."""

    configured = getattr(server, "_direct_model_mode", None)
    return direct_model_mode_enabled() if configured is None else bool(configured)
...
def _stale_project_evidence_notice(stale_status: dict[str, Any], suppressed: int) -> str:
    ...
    return (
        "## Project-source freshness advisory\n\n"
        f"Generated project-source metadata may be stale ({reason}); "
        f"{suppressed} source-derived project RAG row(s) were excluded. "
        "Direct Source/ reads are recommended for current implementation evidence. "
        "Engine and documentation rows below remain retrieval evidence only.\n\n"
    )
...
def _explicit_project_selectors(arguments: dict[str, Any]) -> list[str]:
...
def _direct_source_handoff(
    query: str,
    project_selectors: list[str] | None = None,
) -> dict[str, Any]:
    selectors = [
        str(item).strip()
        for item in (project_selectors or [])
    ...
    if selectors:
        # The Node source capability accepts one exact project per call. Keep
        # the complete originating selector list beside the executable first
        # call so a multi-project query never silently falls back to activeProject.
        args["project"] = selectors[0]
    result = {
    ...
    if selectors:
        result["projectSelectors"] = selectors
    return result
...
def _route_authorization_failure_payload(
    ...
    # Strict-only lazy dependency: Direct startup/catalog listing must not load
    # the legacy task-route policy or its configuration file.
    ...
    same_call_retry = error_code in recovery_codes("sameCallRetryCodes")
    policy_recovery = route_recovery_action(error_code)
    recovery_action_codes = recovery_codes("recoveryActionCodes")
    if error_code in recovery_action_codes:
        payload.setdefault("nextAction", str(policy_recovery["action"]))
        payload.setdefault("nextActionIsTool", bool(policy_recovery["isTool"]))
    recovery_action = (
        error_code in recovery_action_codes
...
def _record_prewrite_gate(
    ...
    *,
    ...
    if _server_direct_mode(server):
        # In Direct Model Mode these validators are optional analysis tools,
        # not task-state transitions or write certificates.
        return None
...
def _handle_unreal_code_sketch_claim_validate(
    ...
    from mcp_tool_compact import compact_code_sketch_payload
...
def _handle_unreal_architecture_reasoning(
    ...
    if not project_root:
        ...
        from mcp_tool_compact import compact_architecture_payload
...
class McpServer:
    def __init__(self, index: Path) -> None:
        ...
        self._direct_model_mode = direct_model_mode_enabled()
        ...
        if not self._direct_model_mode:
            try:
            ...
            try:
                ...
                # Legacy Strict startup remains available in recovery-only mode
                # when reconciliation cannot prove that a route is safe to release.
                pass
    ...
    def notify_tools_list_changed(self) -> None:
        if _server_direct_mode(self):
            # The Direct catalog is process-stable and never follows task state.
            return
        self.send(
            {
                "jsonrpc": "2.0",
    ...
    def error(self, message_id: Any, code: int, message: str) -> None:
        ...
        if request_context and not _server_direct_mode(self):
            try:
    ...
    def tool_result(
        ...
        *,
        ...
        direct_mode = _server_direct_mode(self)
        ...
        if (
            ...
            and tool_name == "unreal_architecture_reasoning"
            and not direct_mode
        ):
        ...
        if isinstance(structured_payload, dict) and not direct_mode:
        ...
        if direct_mode:
            if isinstance(structured_payload, dict):
                public_structured = normalize_direct_payload(
                    structured_payload,
            ...
            if isinstance(public_structured, dict):
                direct_cap = (
                    min(limit, 4_096)
                    if direct_error
                    else min(limit, max(8_000, min(limit // 2, 32_000)))
                )
                public_structured, rendered_direct = bounded_direct_payload(
                    public_structured,
                    direct_cap,
                )
                bounded_error = public_structured.get("ok") is False
                if bounded_error and not direct_error:
                    delivery_key = str(
                        structured_payload.get("deliveryVariantKey") or ""
                    ).strip()
                    if delivery_key:
                        from read_query_history import forget_query_delivery

                        forget_query_delivery(delivery_key)
                if text_was_json or direct_error or bounded_error:
                    public_text = rendered_direct
                direct_error = direct_error or bounded_error
        else:
            public_structured = (
        ...
        payload: dict[str, Any] = {
            ...
            "isError": direct_error if direct_mode else is_error,
        }
        ...
        if method == "initialize":
            ...
            self.result(
                message_id,
                {
                    ...
                    "capabilities": {
                        "tools": {"listChanged": not _server_direct_mode(self)},
    ...
    def all_tool_definitions(self) -> list[dict[str, Any]]:
        if _server_direct_mode(self):
            return direct_tool_definitions(self._all_tool_definitions_unfiltered())
        return self._legacy_strict_tool_definitions()

    def _legacy_strict_tool_definitions(self) -> list[dict[str, Any]]:
    ...
    def tool_catalog_diagnostics(self) -> dict[str, Any]:
        ...
        direct_mode = _server_direct_mode(self)
        if direct_mode:
            registered = self._all_tool_definitions_unfiltered()
            advertised = direct_tool_definitions(registered)
            context: dict[str, Any] = {"status": "disabled"}
        else:
            from task_api import list_tools_route_context
        ...
        return {
            "executionMode": "direct" if direct_mode else "strict",
            "profile": (
                "direct"
                if direct_mode
                else "extended" if extended_tools_enabled() else "essential"
            ),
                ...
                "inputSchema": self._schema(
                    {
                        ...
                        "project": {
                            ...
                            "items": {"type": "string"},
                            "description": (
                                "Optional exact project selectors for this search; "
                                "they do not change activeProject."
                            ),
                        },
                        "top_k": {"type": "integer", "minimum": 1, "maximum": 16, "default": 6},
                        "hybrid": {
    ...
    def search_options_from_args(self, arguments: dict[str, Any], top_k: int) -> tuple[SearchOptions, str]:
        config = load_shared_config()
        explicit = _explicit_project_selectors(arguments)
    ...
    def _run_search_with_diagnostics(
        ...
        *,
        ...
        explicit_projects = _explicit_project_selectors(arguments)
        freshness = stale_status or project_source_stale_status(
            search_mode=mode,
        ...
        selected_project_names = explicit_projects or active_project_names()
        ...
        def fresh_project_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
            ...
            kept = [
                row for row in rows
                if not _is_source_derived_project_row(row, selected_project_names)
            ]
        ...
        if resolved_scope == "mixed" and options.projects:
            ...
            merged = annotate_other_project_rows(merged, selected_project_names)
        ...
        rows = annotate_other_project_rows(rows, selected_project_names)
    ...
    def _authorize_legacy_strict_tool_call(
        self,
        message_id: Any,
        name: Any,
        arguments: dict[str, Any],
    ) -> bool:
        """Run the legacy task/route gateway only in explicit Strict Mode."""
        ...
        elif str(name or "") not in CONTROL_PLANE_TOOLS:
            from task_api import authorize_active_task_tool
    ...
    def handle_tool_call(self, message_id: Any, params: dict[str, Any]) -> None:
        ...
        if direct_mode:
            tool_definitions = self.all_tool_definitions()
            if name not in {tool["name"] for tool in tool_definitions}:
                self.structured_tool_result(
                    message_id,
                    {
                        ...
                        "message": (
                            f"Tool '{name or ''!s}' is not part of the Direct RAG catalog."
                        ),
            ...
            # Legacy profile gate only — route errors must surface as
            # authorization failures, not TOOL_NOT_CALLABLE.
        ...
        if not direct_mode and not self._authorize_legacy_strict_tool_call(
            message_id,
            name,
            arguments,
        ):
        ...
        try:
            ...
            if name == "unreal_open_project_picker":
                ...
                from mcp_tool_compact import compact_agent_plan_payload
    ...
    def handle_agent_session(self, message_id: Any, arguments: dict[str, Any]) -> None:
        ...
        if precheck.get("suppressed"):
            if _server_direct_mode(self):
                structured = duplicate_rag_payload()
                self.tool_result(
                    message_id,
            ...
            source_handoff = _direct_source_handoff(request, explicit_projects)
            structured = {
                "ok": False,
        ...
        payload = {
            ...
            **search_diagnostics,
            ...
            "nextSteps": (
                [
                    "Use the returned evidence or inspect matching project source as needed.",
                    "For edits, read before patching and build to diagnose the result.",
                ]
                if _server_direct_mode(self)
                else [
                    "unreal_get_active_project",
    ...
    def handle_search(self, message_id: Any, arguments: dict[str, Any]) -> None:
        ...
        if pre_delivery.get("suppressed"):
            ...
            source_handoff = _direct_source_handoff(query, explicit_projects)
            structured = {
                "ok": False,
                "errorCode": repeat.get("errorCode") or "RAG_QUERY_REPEAT_BLOCKED",
                "repeatDetected": True,
        ...
        selected_projects = explicit_projects or active_project_names()
        has_project_scope = bool(explicit_projects or active_project)
        ...
        structured: dict[str, Any] = {
            ...
            "directSourcePreferred": stale_status.get("directSourcePreferred", False) or project_miss or (
                zero_result and has_project_scope
            ),
            "doNotRepeatSearch": bool(project_miss or zero_result),
            "matchCount": match_count,
            "projectMatchCount": 0 if project_miss else match_count,
            "activeProjects": selected_projects if (project_miss or has_project_scope) else [],
            **search_diagnostics,
            "projects": explicit_projects,
        }
        if project_miss or (zero_result and has_project_scope):
            structured["requiredNextAction"] = "search_files_then_read_file"
            structured.update(_direct_source_handoff(query, explicit_projects))
            structured["nextSteps"] = [
                "Search the selected project's Source/ for the feature or symbol tokens.",
        ...
        if stale_status.get("stale") or stale_status.get("refreshRecommended"):
            refresh_available = bool(
                extended_tools_enabled()
                and stale_status.get("refreshTargetsSelectedProject", True)
            )
    ...
    def handle_symbol_lookup(self, message_id: Any, arguments: dict[str, Any]) -> None:
        ...
        if (
            ...
            fresh_rows = [
                row for row in rows
                if not _is_source_derived_project_row(
                    row,
                    explicit_projects or active_project_names(),
                )
            ]
        ...
        active_path = str(
            stale_status.get("project")
            or (
                ""
                if explicit_projects
                else load_shared_config().get("activeProject") or ""
            )
        ).strip()
        active_project_path = (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the unsupported historical Unreal workflow MCP.")
    parser.add_argument("--index", default=None, help="Path to rag.sqlite (default: workspace indexPath)")
    parser.add_argument(
        "--legacy-workflow",
        action="store_true",
        help="Explicitly acknowledge the unsupported historical workflow. New installs must use unreal_rag_direct.py.",
    )
...
if __name__ == "__main__":
    ...
    if not args.legacy_workflow:
        print(
            "scripts/unreal_rag_mcp.py is an unsupported historical workflow entry. "
            "Run scripts/unreal_rag_direct.py for Direct Model Mode, or pass "
            "--legacy-workflow only for repository-local historical reproduction.",
            file=sys.stderr,
        )
        raise SystemExit(2)
    if args.index:
    ...
    if direct_model_mode_enabled():
        runtime_component_status = {
            "ok": True,
            "verified": False,
            "reason": "direct_mode_has_no_control_runtime_dependency",
            "running": {"component": "rag", "executionMode": "direct"},
