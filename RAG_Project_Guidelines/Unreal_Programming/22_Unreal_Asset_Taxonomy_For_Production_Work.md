...
| graph_material | unreal_material_metadata | unreal_asset_graph_lookup, unreal_material_claim_validate | **Material·MaterialInstanceConstant만** 그래프/파라미터 익스포트. MF/ML/MPC는 registry만. |
...
| rag_coverage | `registry` |
| `unreal_asset_graph_lookup` | **실패/미지원** — Material/MI 그래프 익스포터 대상이 아님 |
| 권장 조치 | `unreal_rag_search`로 registry 행 조회 → 에디터에서 레이어 열기 → 필요 시 `export_material_metadata.py` 확장 |

Material Function (`MF_*`)과 Material Parameter Collection (`MPC_*`)도 동일하게 **registry only**입니다. 그래프 노드·와이어 주장은 `unreal_material_claim_validate`로 검증할 수 없으며, 익스포트 파이프라인 확장 전까지는 에디터 직접 확인이 필요합니다.
