...
measurements = {(e["modelInputId"], e.get("roundIndex")): e for e in events if e.get("event") == "context_low_water"}
rows = []
for row in data["rounds"]:
    water = measurements.get((row["modelInputId"], row.get("roundIndex")), {})
...
summary = {"completedCalls": len(rows), "actualCompactions": len(compacted),
    ...
    "allBackendPromptMatchesExact": all(r["backendPromptTokens"] == r["finalExactInput"] for r in rows if r["backendPromptTokens"] is not None),
    "usageUnknownCalls": sum(r["backendPromptTokens"] is None for r in rows),
